	...
	private: System::Void bSignin_Click(System::Object^  sender, System::EventArgs^  e) {
		if (tbSLogin->Text->Length <= 3 || tbSLogin->Text->Length > 15 ||
			...
			try {
				...
				polecenie->CommandText = "BEGIN; INSERT INTO uzytkownik(login, haslo) VALUES('"+ tbSLogin->Text +"', PASSWORD('" + tbSPassword1->Text + "')); INSERT INTO dane(imie, nazwisko, email, nr_tel, miasto, ulica, nr_budynku, nr_lokalu, idUzytkownika) VALUES('" + tbSName->Text + "', '" + tbSSurname->Text + "', '" + tbSEmail->Text + "', '" + tbSPNumber->Text + "', '" + tbSCity->Text + "', '" + tbSStreet->Text + "', '" + tbSBNumber->Text + "', '" + tbSANumber->Text + "', LAST_INSERT_ID()); COMMIT;";
